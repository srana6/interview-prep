...
class Codec:
    ...
    def serialize(self, root):
        ...
        while queue:
            ...
            # Node exists, add childs to queue 
            if node:
                data.append(str(node.val))
                queue.append(node.left)
                queue.append(node.right)

            # If a child was null, then serialize as null
            else:
                data.append('null')
    ...
    def deserialize(self, string):
        ...
        emptyValue = 'null'
        data = deque(string.split(','))
        size = len(data)

        # Traverse data to replace 'null' string value with None, if not with integer value
        for i in range(size):
        ...
        first = 0
        root = data[first] if first < size else None

        curr = 0
        child = 1

        # Connect parent with child nodes
        while child < size:            
            if data[curr]:
                data[curr].left = data[child]
                child += 1

                # Validate if can get right child
                if child < size:
                    data[curr].right = data[child]
                    child += 1
